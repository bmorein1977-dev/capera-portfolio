...
- **Competency Levels Management**: Dedicated admin interface (`/admin/competency-levels`) for defining proficiency levels for any competency element before Excel import. This is CRITICAL for the Excel import system, which requires levels to be pre-defined so it can look up level terms (Basic, Intermediate, Advanced, etc.) from Column E and link criteria to the appropriate levels via `levelId`.
- **Inline Level Management**: Proficiency levels can also be managed directly within the Competency Manager (`/admin/competency-manager`). When an element is selected, an inline panel appears showing all defined levels with add/edit/delete capabilities for quick edits after initial setup.
