...
def norm_criteria_type(s: Any) -> str:
    """Map many synonyms to 'knowledge' or 'performance'."""
    if not s: return ""
    t = str(s).strip().lower()
    ...
    if t in {"knowledge","k","uk","u/k","underpinning","under pinning","underpin","underpinning knowledge","under-pinning knowledge"}:
        return "knowledge"
    if t.startswith("underpin") or t.startswith("knowledg"):
        return "knowledge"
    # Performance variants
    if t in {"performance","p","pc","p/c","perf","performance criteria","performance criterion"}:
        return "performance"
...
async def import_xlsx(
    ...
    dry_run: bool = Query(True, description="If true, returns a build preview without writing to DB"),
    ...
    if "subcategory" in values and "subcategory" not in columns:
        columns["subcategory"] = values.index("subcategory")+1
    if "type" in values and "criteria_type" not in columns:
        columns["criteria_type"] = values.index("type")+1
    ...
    for r in ws.iter_rows(min_row=2, values_only=False):
        ...
        raw_t = crittype_c.value if crittype_c else current["criteria_type"]
        t = norm_criteria_type(raw_t)
        if not t:
            t = current["criteria_type"]
        if t not in ("knowledge","performance", None, ""):
            warnings.append({"row": row_index, "message": f"Unknown type '{raw_t}' -> defaulted to 'knowledge'"})
            t = "knowledge"
        current["criteria_type"] = t or current["criteria_type"]
    ...
    if dry_run:
        return {"status":"ok","mode":"dry_run","standards": plan, "count": len(plan), "warnings": warnings}

    return {"status":"ok","mode":"commit_skipped","reason":"Wire commit logic to your ORM (see README).",
            "standards": plan, "count": len(plan), "warnings": warnings}
