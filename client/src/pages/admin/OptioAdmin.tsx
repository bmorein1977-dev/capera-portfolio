...
import { OPTIO_CATEGORIES } from "@shared/schema";
import type { OptioDocument } from "@shared/schema";

const SECTIONS = Array.from(new Set(OPTIO_CATEGORIES.map(c => c.section)));

export default function OptioAdmin() {
  ...
  const { data: documents = [], isLoading } = useQuery<OptioDocument[]>({
    queryKey: ['/api/optio/documents'],
  });

  const documentsByCategory = useMemo(() => {
    const map = new Map<string, OptioDocument[]>();
  ...
  const uploadMutation = useMutation({
    mutationFn: async () => {
      ...
      const res = await fetch('/api/optio/documents', { method: 'POST', credentials: 'include', body: formData });
      if (!res.ok) throw new Error((await res.json().catch(() => ({})))?.error || 'Failed to upload');
      return res.json();
    },
    onSuccess: () => {
      toast({ title: "Uploaded" });
      queryClient.invalidateQueries({ queryKey: ['/api/optio/documents'] });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const res = await fetch(`/api/optio/documents/${id}`, { method: 'DELETE', credentials: 'include' });
  ...
  return (
    <div className="container mx-auto p-6 space-y-6" data-testid="page-optio-admin">
          ...
          {SECTIONS.map(section => (
            <AccordionItem key={section} value={section} className="border rounded-lg px-4">
              ...
              <AccordionContent className="space-y-4">
                {OPTIO_CATEGORIES.filter(c => c.section === section).map(category => {
                              ...
                              <div key={doc.id} className="flex items-center justify-between border rounded-md p-2" data-testid={`row-document-${doc.id}`}>
                                ...
                                <div className="flex items-center gap-1 shrink-0">
                                  <Button variant="ghost" size="sm" asChild data-testid={`button-download-${doc.id}`}>
                                    <a href={`/api/optio/documents/${doc.id}/download`} target="_blank" rel="noopener noreferrer">
      ...
      <Dialog open={!!uploadCategory} onOpenChange={(open) => !open && setUploadCategory(null)}>
        <DialogContent data-testid="dialog-upload-optio-document">
          <DialogHeader>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="optio-title">Title</Label>
              <Input id="optio-title" value={title} onChange={e => setTitle(e.target.value)} placeholder="Defaults to the file name" data-testid="input-optio-title" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="optio-description">Description</Label>
              <Textarea id="optio-description" value={description} onChange={e => setDescription(e.target.value)} rows={2} data-testid="input-optio-description" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="optio-file">File *</Label>
              <Input id="optio-file" type="file" onChange={e => setFile(e.target.files?.[0] || null)} data-testid="input-optio-file" />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setUploadCategory(null)}>Cancel</Button>
            <Button onClick={() => uploadMutation.mutate()} disabled={!file || uploadMutation.isPending} data-testid="button-submit-optio-upload">
